...
import {InstanceOptions} from "../../dom/types";
...
const Default: CarouselOptions = {
    ...
    indicators: {
        items: [],
        activeClasses: 'bg-pink dark:bg-pink',
        inactiveClasses: 'bg-white dark:bg-white hover:bg-pink dark:hover:bg-pink',
    },
    interval: 3000,
    onNext: () => {},
    onPrev: () => {},
...
class Carousel implements CarouselInterface {
    ...
    init(): void {
        if (this._items.length && !this._initialized) {
            ...
            this._indicators.map((indicator: IndicatorItem, position: number) => {
    ...
    slideTo(position: number): void {
        ...
        const rotateItems: RotationItems = {
            left: nextItem.position === 0 ? this._items[this._items.length - 1] : this._items[nextItem.position - 1],
            middle: nextItem,
            right: nextItem.position === this._items.length - 1 ? this._items[0] : this._items[nextItem.position + 1],
        };
        this._rotate(rotateItems);
    ...
    document.querySelectorAll('[data-carousel]').forEach(($carouselEl) => {
        ...
        if ($carouselEl.querySelectorAll('[data-carousel-item]').length) {
            Array.from($carouselEl.querySelectorAll('[data-carousel-item]')).map(($carouselItemEl: HTMLElement, position: number) => {
                items.push({position: position, el: $carouselItemEl});
